...
public class SingleDatabase {
    ...
    int addMovie(String movie_name_text, String movie_description, String theatre_text, String seats_text,
                 String date_text, String day_text) {
        try {
            ...
            int n = Integer.parseInt(day_text);
            String date_arr[] =date_text.split("-",3);
            int p=Integer.parseInt(date_arr[2]);
            ...
            for(int i=0;i<n;i++) {
                ...
                query = "insert into moviedetails(name,theatre,seats,showdate,showtime) values ('"
                        ...
                        + Integer.parseInt(seats_text) + ",'" + date_arr[0]+"-"+date_arr[1]+"-" +Integer.toString(p)+ "','"
                        + "7:30 AM" + "')";
                stm.executeUpdate(query);
    ...
    void updateUserView(DefaultTableModel model, String path) {
        try {
            ...
            while (rs.next()) {
                ...
                Object[] row = { new ImageIcon(new ImageIcon("D:\\JAVA_PROJECT\\Movie-Ticket-Booking-System-main\\model\\Images\\"+name+".jfif").getImage().getScaledInstance(150, 150, Image.SCALE_DEFAULT)), name, description };
                model.addRow(row);
            }
        } catch (Exception e) {
            System.err.println(e.getClass().getName() + ": " + e.getMessage());
